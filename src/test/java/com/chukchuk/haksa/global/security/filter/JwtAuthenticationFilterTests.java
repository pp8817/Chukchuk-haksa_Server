...
class JwtAuthenticationFilterTests {
  ...
  void getStudentProfile_withInvalidAccessToken_returns401() throws Exception {
    ...
    mockMvc
        .perform(get("/api/student/profile").header("Authorization", "Bearer " + token))
        .andExpect(status().isUnauthorized())
        .andExpect(jsonPath("$.success").value(false))
        ...
        .andExpect(
            header()
                .string(
                    "WWW-Authenticate",
                    "Bearer error=\"invalid_token\", error_description=\"TOKEN_INVALID\""));
  }
  ...
  void getAnalyticsId_withWithdrawnUserAccessToken_returns401() throws Exception {
    ...
    mockMvc
        .perform(get("/api/users/analytics-id").header("Authorization", "Bearer " + token))
        .andExpect(status().isUnauthorized())
        .andExpect(jsonPath("$.success").value(false))
        .andExpect(jsonPath("$.error.code").value(ErrorCode.TOKEN_INVALID.code()))
        .andExpect(
