...
class PortalSyncServiceTests {
  ...
  @Mock private InitializePortalConnectionService initializePortalConnectionService;

  @Mock private RefreshPortalConnectionService refreshPortalConnectionService;

  @Mock private SyncAcademicRecordService syncAcademicRecordService;
  ...
  void syncWithPortalKeepsInitialFlowForUnconnectedUser() {
    ...
    when(initializePortalConnectionService.executeWithPortalData(userId, portalData))
  ...
  private static User unconnectedUser(UUID userId) {
    return User.builder().id(userId).email("user@example.com").profileNickname("user").build();
  }
  ...
  private static PortalConnectionResult successConnection(String studentCode) {
    return PortalConnectionResult.success(
        studentCode,
        new PortalConnectionResult.StudentInfo("홍길동", "수원대학교", "컴퓨터학과", studentCode, 4, "재학", 1));
  }
}
